...
// Built-in template ids that now ship as verified GitHub bundles — hidden from
// Others so they don't duplicate the Verified gallery. As each remaining
// template graduates to a verified bundle, add its id here; eventually the
// Others section dissolves entirely.
const VERIFIED_TEMPLATE_IDS = new Set(["ecommerce", "saas", "finance", "medical", "marketing_ads"]);

const BUNDLES_URL = "https://github.com/OWOX/models/tree/main/bundles";

export function LibraryDialog({ onUse, onClose }: Props) {
  const others = [...INDUSTRY_TEMPLATES, ...DATASET_TEMPLATES].filter(t => !VERIFIED_TEMPLATE_IDS.has(t.id));
